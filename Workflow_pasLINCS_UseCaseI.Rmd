...
CorTable <- data.frame(Pathway=PathwayList,cortab,row.names=PathwayList,
                       stringsAsFactors=FALSE)
```
...
##Correlate CP signatures to pathways:
```{r}
Score <- rep(NA,length=length(PathwayList))
for (i in 1:length(PathwayList)) {
 Edges <- AllEdges[AllEdges$Pathway==PathwayList[i],]
 invisible(capture.output(score_tmp <- CalcTPSig(EdgeFrom=Edges$GeneSymbol_a,
                                                 EdgeTo=Edges$GeneSymbol_b,
                                                 interaction=Edges$Interaction,
                                                 data=CP_MCF7,neigen=1)$Score))
 if (!is.null(score_tmp)) {
   Score[i] <- score_tmp
 }
}
names(Score) <- PathwayList
...
```{r}
data.frame(PathwayID=topPathways,PathwayName=AllPathwayName[topPathways,"PathwayName"],
           Correlations=Score[topPathways],stringsAsFactors=FALSE)
```
...
```{r}
scoretab <- matrix(NA,ncol=ncol(CP_MCF7),nrow=length(PathwayList))
for (i in 1:length(PathwayList)) {
 Edges <- AllEdges[AllEdges$Pathway==PathwayList[i],]
 invisible(capture.output(score_tmp <- CalcTPSig(EdgeFrom=Edges$GeneSymbol_a,
                                                 EdgeTo=Edges$GeneSymbol_b,
                                                 ...
                                                 data=CP_MCF7,neigen=1)$Score))
 if (!is.null(score_tmp)) {
   scoretab[i,] <- score_tmp
 }   
}
ScoreTable <- data.frame(Pathway=PathwayList,scoretab,row.names=PathwayList,
                         stringsAsFactors=FALSE)
colnames(ScoreTable)[-1] <- mTOR_CP_SigID
ScoreTable <- ScoreTable[apply(ScoreTable[,-1],1,function(x) sum(is.na(x))!=length(x)),]

source("http://eh3.uc.edu/genomics/GenomicsPortals/ilincs/paslincs/BenchPathway.R")
BenchRes <- BenchPathway(cordata=ScoreTable,pathway=PathwayOfInterest,
                         metadata=LincsMeta$MCF7,pathlist=PathwayList,cutoff=0.95,direction="<")
data.frame(Pathway=PathwayOfInterest,PathwayName=AllPathwayName[PathwayOfInterest,"PathwayName"],
           AUC=BenchRes$AUC,rpAUC=BenchRes$rpAUC,stringsAsFactors=FALSE)
```
